...
class VLLMModel(BaseArchytasModel):
    ...
    @property
    def vllm_model_provider(self) -> str | None:
        if self.model_name is None: return None

        try:
            provider, _ = self.model_name.split("/", maxsplit=1)
            return provider
        except ValueError as e:
            raise ValueError(f"Failed to parse model provider for vLLM model name '{ self.model_name }'.") from e
        
    def is_ollama_model(self) -> bool:
        return self.vllm_model_provider == "meta-llama"
    
    def is_google_model(self) -> bool:
        return self.vllm_model_provider == "google"
    ...
    def configure_prompt_instructions(self) -> None:
        """ Currently, it seems that models like to ignore tool calling instructions in their chat templates.
        Haven't gotten to the root of the issue quite yet, so this is a temporary patching measure to ensure
        models behave properly with regards to tool invocations.
        """
        if self.is_ollama_model():
            # This is copied from the `MODEL_PROPMT_INSTRUCTIONS` of arcyhtas' `OllamaModel`
            self.MODEL_PROMPT_INSTRUCTIONS = """
...
"""
        elif self.is_google_model():
            pass
        else:
            logger.info(
                "This may lead to instability, particularly in its usage of tools. "
                f"Proceeding to use '{ self.model_name }' without extra instructions."
            )


    def initialize_model(self, **kwargs):
        self.load_available_models()
        self.configure_prompt_instructions()
